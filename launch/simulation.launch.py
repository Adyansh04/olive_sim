...
def load_world_config(context, *args, **kwargs):
    ...
    # Gazebo Harmonic. Headless (server-only) mode skips the Qt GUI - use it for CI,
    # robots, or when a snap-polluted environment breaks the GUI. Sensors still render offscreen.
    if LaunchConfiguration('headless').perform(context) == 'true':
        gz_args = ['-s -r --headless-rendering -v 1 ', world_file]
    else:
...
def generate_launch_description():
    ...
    declare_headless_cmd = DeclareLaunchArgument(
        'headless',
        default_value='false',
        description='Run Gazebo server-only without the GUI (headless; sensors still render offscreen)'
    )
